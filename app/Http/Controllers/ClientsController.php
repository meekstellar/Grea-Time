...
class ClientsController extends Controller
{
        ...
        if (!empty($request->export)) {
            if ($request->export == 'xls') {
                return Excel::download(
                    new xlsExport($request->date_or_period, $request->w, $service),
                    'export_'.date('Y-m-d-H-i-s').'.xlsx',
                );
            }
            if ($request->export == 'pdf') {
                return Excel::download(
                    new pdfExport($request->date_or_period, $request->w, $service),
                    'export_'.date('Y-m-d-H-i-s').'.pdf',
                );
            }
        } else {
    ...
    public function editClient(Request $request)
    {
        ...
        if (!empty($request->delete_user)) {
            ...
            return redirect($lastUrlForReditect)->with(
                'status',
                'Данные клиента <b>'.$request->name.'</b> были обновлены.',
            );
        }
    }
    ...
    public function setFee(Request $request)
    {
        ...
        return response()->json(
            ['status' => true, 'messages' => 'Гонорар установлен для клиента '.$user->name.$ClientsFees->id.''],
        );
    }
    ...
    public function clientSettings(Request $request): View
    {
        if (auth()->user()->role !== 'worker') {
            Event::listen(BuildingMenu::class, function (BuildingMenu $event) {
                // Add some items to the menu...
