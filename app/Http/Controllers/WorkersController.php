...
class WorkersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $date_or_period_with_secounds[] = new Carbon(
            (!empty($date_or_period[1]) ? $date_or_period[1] : $date_or_period[0]),
        ); // Final date
        ...
        $workers = User::query()
            ...
            ->active()
            ->with([
                'clientHours' => function ($query) use ($date_or_period_with_secounds) {
                    $query->whereBetween(
                        'created_at',
                ...
                'clients',
                'salary',
                'clientHoursByClient' => function ($query) use ($date_or_period_with_secounds) {
                    $query->whereBetween(
                        'created_at',
                        [$date_or_period_with_secounds[0], $date_or_period_with_secounds[1]],
                    );
            ...
            ->whereHas('clientHours', function ($query) use ($date_or_period_with_secounds) {
                $query->whereBetween(
                    'created_at',
                    [$date_or_period_with_secounds[0], $date_or_period_with_secounds[1]],
                );
        ...
        $workersWithoutTime = User::query()
            ...
            ->active()
            ->with(['clients'])
            ->whereDoesntHave('clientHours', function ($query) use ($date_or_period_with_secounds) {
                $query->whereBetween(
                    'created_at',
                    [$date_or_period_with_secounds[0], $date_or_period_with_secounds[1]],
                );
        ...
        return view('workers')->with([
            'workers' => $workers->get(),
            'workersWithoutTime' => $workersWithoutTime->get(),
    ...
    public function worker(Request $request)
    {
        ...
        $date_or_period_with_secounds[1]->addHour(23)->addMinutes(59)->addSeconds(59);
        $WorkerClientHoursArray = WorkerClientHours::whereBetween(
            "created_at",
            [$date_or_period_with_secounds[0], $date_or_period_with_secounds[1]],
        )
        ...
        $WorkerClientHoursArray_Last = WorkerClientHours::whereBetween(
            "created_at",
            [$date_or_period_with_secounds_last[0], $date_or_period_with_secounds_last[1]],
        )
            ->where("worker_id", auth()->user()->id)->get()->sortByDesc('created_at');

        $user_ids = Auth::user()->get_connect_clients_id();

        $users['clients'] = User::WhereIn('id', $user_ids)->where('role', 'client')->where('active', 1)->get()->sortBy(
            'name',
        );
    ...
    public function saveWorker(Request $request)
    {
        if (date('H', time()) >= 17 && date('H', time()) <= 23) {
            ...
            foreach ($request->clients as $client_id => $hours) {
                ...
                $WorkerClientHours = WorkerClientHours::whereBetween(
                    "created_at",
                    [$date_or_period_with_secounds[0], $date_or_period_with_secounds[1]],
                )
            ...
            return redirect('worker')->with(
                'error',
                'Невозможно сохранить часы работы,
    ...
    public function changeClientsHours(Request $request)
    {
        ...
        return response()->json(
            ['status' => true, 'messages' => 'Установлены часы работы для <br><b>'.$wc->client()->name.'</b>'],
        );
    }
    ...
    public function addClientHours(Request $request)
    {
        ...
        return redirect($lastUrlForReditect.'#u'.$request->worker_id)
            ->with(
                'status',
                'Установлены часы работы для сотрудника <b>'.$wc->worker()->name.'</b> (клиент: <b>'.$wc->client(
                )->name.'</b>).',
            );
    }
    ...
    public function addRestDay(Request $request)
    {
        ...
        return response()->json(
            [
                'status' => true,
                'message' => 'Установлен <b>'.$wc->client(
                    )->name.'</b> в день '.$request->date_or_period.' для сотрудника <b>'.$wc->worker(
                    )->name.'</b>.<br><br><b>Обновите страницу.</b>',
            ],
        );
    }
    ...
    public function editWorkerFromClient(Request $request)
    {
        ...
        if (!empty($request->delete_user)) {
            ...
            return redirect($lastUrlForReditect)->with(
                'status',
                'Данные сотрудника <b>'.$request->name.'</b> были обновлены.',
            );
        }
    }
