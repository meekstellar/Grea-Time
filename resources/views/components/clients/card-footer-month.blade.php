@php
    $all_clients_hours = $WorkerClientHours->where('client_id',$wc->client_id)->sum('hours')*1000;
...
<table class="table table-sm client-table mb-0">
    <tbody>
        <tr>
            ...
            <td style="border-top: 0; text-align: right;"><span class="setedCostPrice">{{ $all_clients_hours }}</span> {{ $currency }}</td>
        </tr>
        <tr>
            <td colspan="2">OPEX (35%)</td>
            <td style="text-align: right;"><span class="setedOPEX">{{ round($OPEX,0) }}</span> {{ $currency }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td style="text-align: right; font-weight: bold;"><span class="seted_profit">{{ round($profit,0) }}</span> {{ $currency }}</td>
        </tr>
        <tr>
